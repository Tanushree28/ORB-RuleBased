...
class ExpandedORBBacktest:
    ...
    def load_data(self, symbol, interval="5m"):
        ...
        for suff in (interval, "15m", "1h"):
            fp = f"data/{safe_symbol}_{suff}.csv"
...
def categorize_symbols(results):
    ...
    for r in results:
        ...
        elif symbol.endswith("=F"):  # 👈 futures like MNQ=F, NQ=F, ES=F
            categories["futures"].append(r)
        elif symbol.endswith("=X") or symbol in forex_tickers:  # 👈 forex
...
def create_comprehensive_charts(results, categories):
    ...
    colors = [
        "darkgreen" if r > 50 else "green" if r > 20 else "lightgreen" for r in returns
    ]
    ...
    ax3.axvline(
        x=33.33, color="red", linestyle="--", linewidth=2, label="Min for 2x TP"
    )
    ax3.axvline(
        x=np.mean(win_rates),
        color="green",
    ...
    scatter = ax4.scatter(
        pf_values, returns_all, alpha=0.6, c=returns_all, cmap="RdYlGn", s=50
    )
    ...
    colors5 = [
        "darkgreen" if r > 50 else "green" if r > 0 else "red" for r in all_returns
    ]
    ...
    ax5.set_title(
        f"Returns for All {len(results)} Symbols", fontsize=12, fontweight="bold"
    )
    ...
    plt.suptitle(
        "ORB Strategy - Comprehensive Backtest Results (2x TP)",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
...
def main():
    ...
    for symbol in symbols:
        ...
        if result and result["total_trades"] > 0:
            ...
            print(
                f"{status} {symbol:8} - {result['total_trades']:3} trades, "
