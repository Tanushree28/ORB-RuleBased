...
class PolygonDownloader:
    def __init__(self, config_path: str = str(CONFIG_PATH)):
        ...
        # Create output directory (anchored to repo root)
        out_dir_cfg = self.config["output"]["directory"]
        self.output_dir = str((PROJECT_ROOT / out_dir_cfg).resolve())
    ...
    def download_all_symbols(self) -> Dict[str, pd.DataFrame]:
        ...
        print(
            f"Rate limit: {self.config['rate_limit']['requests_per_minute']} requests/minute"
        )
...
def main():
    ...
    if not CONFIG_PATH.exists():
        ...
        print(f"Please ensure {CONFIG_PATH} exists with your API key")
        print("=" * 80)
        return

    try:
        # Initialize downloader
        downloader = PolygonDownloader(config_path=str(CONFIG_PATH))
