...
# --- Anchored paths ---
BASE_DIR = Path(__file__).resolve().parent  # .../ORB/Polygon
PROJECT_ROOT = BASE_DIR.parent  # .../ORB
CONFIG_PATH = PROJECT_ROOT / "configs" / "polygon_config.yaml"

# outputs go here:
REPORTS_DIR = PROJECT_ROOT / "reports" / "polygon"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

# data directory
DATA_DIR = PROJECT_ROOT / "data" / "polygon"
DATA_DIR.mkdir(parents=True, exist_ok=True)


class ExtendedORBBacktest:
    def __init__(self, initial_capital=10000, data_dir=DATA_DIR):
    ...
    def calculate_orb(self, data, date):
        ...
        return {"high": orb_high, "low": orb_low, "range": orb_range, "date": date}
    ...
    def backtest_symbol(self, symbol, start_date=None, end_date=None):
            ...
            for idx, bar in trading_data.iterrows():
                ...
                if position is None:
                    ...
                    if bar["Close"] > orb["high"] and bar["Open"] <= orb["high"]:
                        ...
                        shares = (
                            risk_amount / risk_per_share if risk_per_share > 0 else 0
                        )
                ...
                # Track equity curve
                equity_curve.append({"date": idx, "capital": capital})
        ...
        if len(trades) > 0:
            ...
            avg_loss = (
                abs(trades_df[trades_df["pnl"] < 0]["pnl"].mean()) if losses > 0 else 0
            )

            gross_wins = trades_df[trades_df["pnl"] > 0]["pnl"].sum() if wins > 0 else 0
            gross_losses = (
                abs(trades_df[trades_df["pnl"] < 0]["pnl"].sum()) if losses > 0 else 1
            )
            ...
            if len(trades_df) > 1:
                ...
                sharpe = (
                    (trades_df["returns"].mean() / trades_df["returns"].std())
                    * np.sqrt(252)
                    if trades_df["returns"].std() > 0
                    else 0
                )
            else:
                sharpe = 0
...
def analyze_by_period(results_by_period):
    ...
    if win_rates_by_period:
        bp = ax2.boxplot(
            win_rates_by_period.values(), labels=win_rates_by_period.keys()
        )
    ...
    for period, results in results_by_period.items():
        if results:
            drawdowns = [
                abs(r["max_drawdown"]) for r in results if r["max_drawdown"] != 0
            ]
            if drawdowns:
    ...
    if drawdowns_by_period:
        bp2 = ax4.boxplot(
            drawdowns_by_period.values(), labels=drawdowns_by_period.keys()
        )
        ax4.set_title(
            "Max Drawdown Distribution by Period", fontsize=14, fontweight="bold"
        )
    ...
    if pf_by_period:
        ...
        ax5.set_title(
            "Profit Factor Distribution by Period", fontsize=14, fontweight="bold"
        )
    ...
    plt.suptitle(
        "ORB Strategy - Extended Period Analysis (2 Years)",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
    ...
    # Save the figure
    out_img = REPORTS_DIR / "extended_period_analysis.png"
    plt.savefig(out_img, dpi=100, bbox_inches="tight")
    print(f"✓ Period analysis saved to {out_img}")
...
def main():
    ...
    data_dir_cfg = config["output"]["directory"]  # e.g., "data/polygon"
    data_dir = str((PROJECT_ROOT / data_dir_cfg).resolve())
    ...
    if os.path.exists(symbols_file):
        ...
        symbols = config["symbols"]["stocks"] + [
            s for s in config["symbols"].get("forex", [])
        ]
    ...
    for period_name, start_date, end_date in periods:
        ...
        for symbol in symbols:
            ...
            if result and result["total_trades"] > 0:
                ...
                print(
                    f"{status} {symbol:10} - {result['total_trades']:3} trades, "
    ...
    out_csv = REPORTS_DIR / "extended_backtest_results.csv"
    ...
    legacy_reports = PROJECT_ROOT / "reports" / "comprehensive_results_2x.csv"
    if legacy_reports.exists():
        sixty_day_df = pd.read_csv(legacy_reports)
        ...
        for symbol in symbols[:20]:  # Top 20 symbols
            # Get 2-year result
            two_year = next(
                (r for r in full_period_results if r["symbol"] == symbol), None
            )
            # Get 60-day result
            sixty_day = sixty_day_df[sixty_day_df["symbol"] == symbol].to_dict(
                "records"
            )
