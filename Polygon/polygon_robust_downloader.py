...
# --- Anchored paths (works whether this file is in ORB/ or ORB/Polygon/) ---
...
class RobustPolygonDownloader:
    def __init__(self, config_path: str = str(CONFIG_PATH)):
        ...
        # Output dir anchored to repo root
        out_cfg = self.config["output"]["directory"]  # e.g., "data/polygon"
        self.output_dir = str((PROJECT_ROOT / out_cfg).resolve())
        os.makedirs(self.output_dir, exist_ok=True)

        # Track progress
        self.progress_file = str(Path(self.output_dir) / "download_progress.json")
    ...
    def download_symbol_with_retry(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.info(
                    f"Downloading {symbol} (attempt {attempt + 1}/{self.max_retries})..."
                )
                ...
                try:
                    ...
                    if "429" in str(api_error):
                        logger.warning(
                            f"Rate limit hit for {symbol}, waiting {self.retry_delay}s..."
                        )
                ...
                if "429" in error_msg or "rate" in error_msg.lower():
                    logger.warning(
                        f"Rate limit error for {symbol}, waiting {self.retry_delay}s before retry..."
                    )
                    time.sleep(self.retry_delay)
                elif attempt == self.max_retries - 1:
                    logger.error(
                        f"✗ Failed to download {symbol} after {self.max_retries} attempts: {error_msg}"
                    )
                    return None
                else:
                    logger.warning(
                        f"Error on attempt {attempt + 1} for {symbol}: {error_msg}"
                    )
                    time.sleep(self.base_delay)
    ...
    def download_priority_symbols(self) -> Dict[str, pd.DataFrame]:
        ...
        priority_symbols = [
            ...
            "MNQ=F",  # Futures high performer
            "NQ=F",  # Futures within top 10
            "PEP",
            "XLF",
            "AMD",
            "BAC",
        ...
        avail_file = Path(self.output_dir) / "available_symbols.txt"
        if avail_file.exists():
            with open(avail_file, "r", encoding="utf-8") as f:
                all_symbols = [ln.strip() for ln in f if ln.strip()]
        else:
            # Fallback to config universe if the file isn't there yet
            stocks = self.config["symbols"].get("stocks", []) or []
            forex = self.config["symbols"].get("forex", []) or []
            all_symbols = list(
                dict.fromkeys([*stocks, *forex])
        ...
        for symbol in all_symbols:
            if (
                symbol not in symbols_to_download
                and symbol not in self.completed_symbols
            ):
                symbols_to_download.append(symbol)
...
def verify_downloads():
    ...
    for file in sorted(files):
        ...
        if len(df) > 0:
            ...
            date_range = f"{dt_col.min().date()} to {dt_col.max().date()}"
            symbols_data.append(
                {"Symbol": symbol, "Bars": len(df), "Date Range": date_range}
            )
    ...
    avail_path = Path(data_dir) / "available_symbols.txt"
    with open(avail_path, "w", encoding="utf-8") as f:
        for symbol in available_symbols:
            f.write(f"{symbol}\n")

    print(
        f"\n✓ Updated available symbols list: {avail_path} ({len(available_symbols)} symbols)"
    )
