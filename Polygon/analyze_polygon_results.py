...
# ── Anchored paths (works from ORB/ or ORB/Polygon/)
BASE_DIR = Path(__file__).resolve().parent
REPO_ROOT = BASE_DIR if (BASE_DIR / "configs").exists() else BASE_DIR.parent
REPORTS_DIR = REPO_ROOT / "reports" / "polygon"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

IN_CSV = REPORTS_DIR / "extended_backtest_results.csv"
OUT_IMG = REPORTS_DIR / "polygon_detailed_analysis.png"
...
def load_and_analyze():
    ...
    if not IN_CSV.exists():
        raise FileNotFoundError(
            f"Extended results not found at {IN_CSV}. "
            "Run backtest_extended_polygon.py first."
        )
    ...
    for period in df["period"].unique():
        ...
        for _, row in period_data.iterrows():
            print(
                f"{row['symbol']:6} | Trades: {row['total_trades']:4} | "
    ...
    for _, row in full_period.iterrows():
        ...
        print(
            f"{row['symbol']:6} | Total: {row['return_pct']:14,.1f}% | "
    ...
    print("\nMaximum Drawdowns:")
    for _, row in full_period.iterrows():
        denom = abs(row["max_drawdown"]) if row["max_drawdown"] != 0 else np.nan
        radj = (row["return_pct"] / denom) if pd.notna(denom) else np.nan
        print(
            f"{row['symbol']:6} | Max DD: {row['max_drawdown']:6.1f}% | "
            f"Risk-Adjusted Return: {radj:8.1f}x"
        )

    print("\nProfit Factors:")
    for _, row in full_period.iterrows():
        print(
            f"{row['symbol']:6} | Profit Factor: {row['profit_factor']:5.2f} | "
            f"Win Rate: {row['win_rate'] * 100:5.1f}%"
        )
    ...
    for _, row in full_period.iterrows():
        ...
        print(
            f"{row['symbol']:6} | Total Trades: {row['total_trades']:4} | "
...
def create_detailed_analysis_charts(df):
    ...
    for i, symbol in enumerate(symbols):
        ...
        for period in periods:
            period_val = symbol_data[symbol_data["period"] == period][
                "return_pct"
            ].values
    ...
    bars = ax4.bar(
        ...
        pf_values,
        color=[
            "green" if pf > 2 else "orange" if pf > 1.5 else "red" for pf in pf_values
        ],
    )
    ...
    sharpe_values = [row["sharpe_ratio"] for _, row in full_period.iterrows()]
    bars = ax5.bar(
        symbols_list,
        sharpe_values,
        color=[
            "darkgreen" if s > 3 else "green" if s > 2 else "orange" if s > 1 else "red"
    ...
    summary_text += "Best Performer: " + f"{best['symbol']}\n"
    ...
    plt.suptitle(
        "ORB Strategy - Detailed Polygon Data Analysis",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
    ...
    print(f"\n✓ Detailed analysis saved to {OUT_IMG}")
