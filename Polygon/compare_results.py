...
# Outputs: ALWAYS use reports/polygon
...
def load_results():
    ...
    # Load 60-day results (optional, legacy location)
    sixty_path = PROJECT_ROOT / "reports" / "comprehensive_results_2x.csv"
    if sixty_path.exists():
        sixty = pd.read_csv(sixty_path)
    ...
    # Load extended results (saved by backtest into reports/polygon)
    extended_path = REPORTS_DIR / "extended_backtest_results.csv"
    if extended_path.exists():
        ...
        print(f"✗ Extended results not found at {extended_path}; nothing to plot.")
...
def create_comparison_report(results):
    ...
    out_img = REPORTS_DIR / "comparison_analysis.png"
    plt.savefig(out_img, dpi=100, bbox_inches="tight")
    print(f"\n✓ Comparison analysis saved to {out_img}")
...
def main():
    ...
    print("=" * 80)
    print("\nReports generated:")
    print(f"  - {REPORTS_DIR / 'comparison_analysis.png'}")
    print(f"  - {REPORTS_DIR / 'extended_period_analysis.png'}")
    print(f"  - {REPORTS_DIR / 'extended_backtest_results.csv'}")
