"""Run the systematic ORB sweep and build a comprehensive visual report."""
...
from systematic_backtest import run_backtests, summarise_results
from visualize_systematic_backtest import (
    REPORTS_DIR,
    build_combo_summary,
...
def _category_mapping(row: Dict[str, str]) -> str:
    """Normalise category labels for plotting."""

    category = (row.get("category") or "").strip()
    if category:
        return category
    # Fallback buckets if the feed does not provide categories
...
def _summarise_categories(results_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate returns and win rates by category."""

    if results_df.empty:
        return pd.DataFrame()

    tmp = results_df.copy()
    tmp["category_norm"] = tmp.apply(_category_mapping, axis=1)

    summary = (
        tmp.groupby("category_norm")
        .agg(
...
def _combo_identifier(row: pd.Series) -> str:
    """Build a readable label for a parameter combination."""
...
def _create_overview_figure(
    ...
    """Build the comprehensive multi-panel figure and save it to *save_path*."""
    ...
    if top_combos.empty:
        ...
        returns = top_combos["avg_return"].to_list()
        colors = [
    ...
    if category_summary.empty:
        ...
        bars = ax2.bar(
            category_summary["category_norm"],
        ...
        ax2.set_xticklabels(category_summary["category_norm"], rotation=45, ha="right")
    ...
    # 3. Win rate distribution across all runs
    ...
    # 4. Profit factor vs returns scatter
    ...
    if mask.sum() == 0:
        ...
        scatter = ax4.scatter(
            pf[mask], returns[mask], c=returns[mask], cmap="RdYlGn", alpha=0.6, s=40
        )
    ...
    if not category_summary.empty:
        ...
        best_category_text = (
            f"{best_category['category_norm']} ({best_category['avg_return']:.1f}% | "
            f"n={int(best_category['total_symbols'])})"
        )

    summary = f"""
    SYSTEMATIC ORB SWEEP SUMMARY
    {"=" * 45}
    ...
    fig.suptitle(
        "Systematic ORB Strategy – Comprehensive Overview",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
...
def main() -> None:
    """Execute the systematic sweep and render a comprehensive dashboard."""

    print("=" * 90)
    print("SYSTEMATIC ORB BACKTEST – COMPREHENSIVE REPORT")
    print("=" * 90)
    ...
    chart_path = reports_dir / "systematic_comprehensive_overview.png"
    _create_overview_figure(
        results_df, combo_summary, symbol_summary, category_summary, chart_path
    )

    print(f"✓ Comprehensive systematic report saved to {chart_path}")
