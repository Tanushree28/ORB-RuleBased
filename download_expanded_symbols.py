...
def download_expanded_data():
    ...
    print("Categories: Tech, Consumer, Finance, Healthcare, Energy, Forex, ETFs")
    print("-" * 80)
...
def verify_downloads():
    ...
    for file in data_files:
        if file.endswith("_5m.csv") or file.endswith("_15m.csv"):
            path = os.path.join("data", file)
            try:
                # Read just the header/first row to get the exact symbol we saved
                df_head = pd.read_csv(path, nrows=1)
                symbol_from_csv = df_head.get("Symbol", pd.Series([None])).iloc[0]
                if isinstance(symbol_from_csv, str) and symbol_from_csv:
                    symbols_with_intraday.add(symbol_from_csv)
                else:
                    # Fallback: derive from filename using the RIGHT-split
                    safe_symbol = file.rsplit("_", 1)[0]  # strip only the "_5m"/"_15m"
                    # convert back only '=' substitutions we made when saving
                    symbol_guess = safe_symbol.replace("_", "=")
                    symbols_with_intraday.add(symbol_guess)
            except Exception:
                # Last-ditch fallback (still better than split('_')[0])
                safe_symbol = file.rsplit("_", 1)[0]
                symbols_with_intraday.add(safe_symbol)
