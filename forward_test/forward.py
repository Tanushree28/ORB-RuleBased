...
def create_contract():
    return Future(symbol=SYMBOL, lastTradeDateOrContractMonth=EXPIRY,
...
def place_bracket_order(ib, contract, qty, entry_price, or_range, is_long):
    ...
    tp_price = entry_price + TP_MULTIPLIER*or_range if is_long else entry_price - TP_MULTIPLIER*or_range
    parent = LimitOrder(action, qty, entry_price, transmit=False)
    stop  = StopOrder('SELL' if is_long else 'BUY', qty, sl_price,
                      parentId=parent.orderId, transmit=False)
    takep = LimitOrder('SELL' if is_long else 'BUY', qty, tp_price,
                       parentId=parent.orderId, transmit=True)

    print(f"Submitting {action} at {entry_price:.2f}, SL={sl_price:.2f}, TP={tp_price:.2f}")
...
def main():
    ...
    util.startLoop()
    ib = IB(); ib.connect(HOST, PORT, CLIENT_ID)
    ...
    # --- account info
    net_liq = float(next(x.value for x in ib.accountSummary() if x.tag=="NetLiquidation"))
    ...
    or_bars = df[(df['datetime'].dt.time>=OPEN_START)&(df['datetime'].dt.time<OPEN_END)]
    if or_bars.empty:
        print("No opening bars collected."); ib.disconnect(); return
    ...
    # wait until 9:50 ET
    target = datetime.now(pytz.timezone('US/Eastern')).replace(hour=9,minute=50,second=0,microsecond=0)
    print("Waiting for 9:45–9:50 bar to close...")
    while datetime.now(pytz.timezone('US/Eastern')) < target:
        ib.sleep(1)

    # check breakout
    df = util.df(bars); df['datetime']=pd.to_datetime(df['date']).dt.tz_convert('US/Eastern')
    latest = df.iloc[-1]; price = latest['close']
    print(f"9:45–9:50 bar closed at {price:.2f}")

    if price > or_high:
        print("Long breakout detected."); place_bracket_order(ib, contract, qty, or_high, or_range, True)
    elif price < or_low:
        print("Short breakout detected."); place_bracket_order(ib, contract, qty, or_low, or_range, False)
    else:
